...
# output old testament kjv with morph module for xiphos crosswire sword
...
def convertverse(verse):
  ...
  for word in words:
    wo=re.match(r'(.+?)<',word)
    pars=re.findall(r'<WT?([HM]\d+)>',word)
    strongs=u''
    morphs=u''
    for item in pars:
      if item.startswith('M'):
        strongs=strongs+'strong:'+item+' '
      #else:
       # morphs=morphs+'rmacplus:'+item+' '
    verss=verss+'<w lemma="'+strongs.strip()+'" morph="'+morphs.strip()+'">'+wo.group(1)+'</w>'
  return verss
